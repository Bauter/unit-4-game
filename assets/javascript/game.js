...
$(document).ready(function() {
    ...
    //console.log("this function is active: " + resultClear);
    ...
    let resultCheck = function() {
        if (userScore === randomNum) {   
            wins++;
            $("#result").text("YOU WON last round! Try again!");
           // alert ("YOU WON!"); // used an alert because unable to display and clear out result div text upon win or lose 
            $("#wins").text("wins: " + wins);
            reset(); // attempt to delay reset to allow result div to display win/lose and then reset and clear
     ...
     let reset = function() {
        ...
        /*let resultClear = function() { // an attempt to create a function that can be called to clear out the result win/lose result div upon reset to avoid using alerts....cant seem to make it work, tried multiple placements (click event, reset function, etc), tried empty, remove, text, clear. and/or prevents original div win/loose display in resultCheck. 
    
            $("#result").text(" "); // appears to override .text method written in conditional statement, scope issue?
        };
